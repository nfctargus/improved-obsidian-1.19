...
public class ObsideriteInfusingStationBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (blockEntity instanceof ObsideriteInfusingBlockEntity) {
                ItemScatterer.spawn(world, pos, (ObsideriteInfusingBlockEntity)blockEntity);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new ObsideriteInfusingBlockEntity(pos, state);
    }

    @Nullable
    @Override
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, ModBlockEntities.OBSIDERITE_INFUSING_STATION, ObsideriteInfusingBlockEntity::tick);
    }
}